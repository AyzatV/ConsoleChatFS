...
Файл **MakeVersion** позволяет узнать версию утилиты **make**. Поскольку этот файл имеет нестандартное имя, утилиту **make** следует использовать с ключом **`-f`**, после которого нужно написать имя этого файла.
